...
sub reduceAPE {
    ...
    foreach my $ap (@aps) {
        ...
        foreach my $lk (@l) {
            ...
            foreach my $z ($lk->getZeros()) {
                ...
                if (scalar($z->getPossible()) == 2) {
                    ...
                    if (defined($opav{join(',', $z->getPossible())})) {$opav{join(',', $z->getPossible())} = [ $z->getRC(), $z ];}
                    if (defined($opav{join(',', reverse $z->getPossible())})) {$opav{join(',', reverse $z->getPossible())} = [ $z->getRC(), $z ];}
                }
        ...
        foreach my $tset (@t) {
            ...
            my %h0;
            map {$h0{$_} = 1;} map {(split(/,/, $_))[0]} keys %apv;
            my %h1;
            map {$h1{$_} = 1;} map {(split(/,/, $_))[1]} keys %apv;
            ...
            if ($answer) {
                foreach my $p ($ap->[0]->getPossible()) {$ap->[0]->setColor('blue', $p);}
                foreach my $p ($ap->[1]->getPossible()) {$ap->[1]->setColor('blue', $p);}
